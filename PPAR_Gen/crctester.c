...
static int order = 32;
static unsigned long polynom = 0x4c11db7;
static int direct = 1;
static unsigned long crcinit = 0xffffffff;
static unsigned long crcxor = 0xffffffff;
static int refin = 1;
static int refout = 1;
...
int boCalculateCRC(int iDLC, unsigned char * aucData, int iOrder, unsigned long ulPolynom, int boDirect, unsigned long ulCRCInit, unsigned long ulCRCXOR, int boRefIn, int boRefOut, unsigned long * pulCRCResult)
{
	...
	*/
	...
	if (!(order & 7))
	{ // "Gerader" CRC
		*pulCRCResult = crctablefast(aucData, iDLC);
	}
	else
	{ // "Krummer" CRC
		*pulCRCResult = crcbitbybitfast(aucData, iDLC);
	}
